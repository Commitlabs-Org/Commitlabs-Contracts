...
| add_authorized_contract(caller, contract_address) -> Result                                                                                    | Whitelist an address as an authorized minter. | Admin require_auth. | Emits AuthorizedContractAdded event.  |
| remove_authorized_contract(caller, contract_address) -> Result                                                                                 | Remove an address from the minter whitelist.  | Admin require_auth. | Emits AuthorizedContractRemoved event.|
| is_authorized(contract_address) -> bool                                                                                                        | Check if address may call mint().  | View.               | Returns true for admin, core contract, or whitelisted address. |
| mint(caller, owner, commitment_id, duration_days, max_loss_percent, commitment_type, initial_amount, asset_address, early_exit_penalty) -> Result<u32> | Mint NFT for a commitment.         | **caller.require_auth()** — caller must be admin, core contract, or an address whitelisted via `add_authorized_contract`. | Validates inputs and uses reentrancy guard. Uses SafeMath for supply increment. |
