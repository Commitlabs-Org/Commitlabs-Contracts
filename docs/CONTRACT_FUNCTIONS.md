...
| allocate(caller, commitment_id, amount, strategy) -> Result<AllocationSummary> | Record virtual allocation across pools. | caller.require_auth. | Records results in persistent storage; does NOT move assets (non-custodial). |
| rebalance(caller, commitment_id) -> Result<AllocationSummary>                  | Update existing allocation records.     | caller.require_auth. | Requires caller matches original owner; record-keeping only.                 |
