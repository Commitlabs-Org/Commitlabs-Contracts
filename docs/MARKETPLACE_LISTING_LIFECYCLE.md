...
- **Preconditions**: `price > 0`, no existing listing for `token_id`, `seller` owns an active token in the configured `commitment_nft` contract
...
- **Reentrancy guard**: yes
- **Preconditions**: `amount > 0`, no existing offer from `offerer` for this token, existing listing/auction seller still owns an active token when one exists
...
- **Auth**: `seller.require_auth()`
- **Reentrancy guard**: yes
- **Preconditions**: `starting_price > 0`, `duration_seconds > 0`, no existing auction, `seller` owns an active token in the configured `commitment_nft` contract
...
| 4 | `NotSeller` | `cancel_listing` by non-lister; listing/auction creation when seller does not own the NFT |
| 5 | `NFTNotActive` | Listing/auction creation when NFT is inactive |
...
| 10 | `NFTContractError` | Configured NFT contract ownership/activity lookup failed |
