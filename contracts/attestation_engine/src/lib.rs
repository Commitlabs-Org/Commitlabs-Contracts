...
use shared_utils::batch::{BatchError, BatchMode, BatchProcessor, BatchResultVoid};
...
// 1. Reentrancy protection
    ...
    pub fn get_health_metrics(e: Env, commitment_id: String) -> HealthMetrics {
        ...
        let (fees_generated, volatility_exposure, last_attestation_stored, compliance_score_stored) =
            stored
                .map(|m| {
                    (
                        m.fees_generated,
        ...
        HealthMetrics {
            ...
            fees_generated,
            volatility_exposure,
            last_attestation: last_attestation.max(last_attestation_stored),
            compliance_score: if compliance_score != 0 {
                compliance_score
            } else {
                compliance_score_stored
            },
        }
    }
    ...
    pub fn record_drawdown(
        ...
        let atts_key = DataKey::Attestations(commitment_id.clone());
        let mut attestations: Vec<Attestation> = e
            .storage()
            .persistent()
            .get(&atts_key)
    ...
    pub fn calculate_compliance_score(e: Env, commitment_id: String) -> u32 {
        ...
        e.events().publish(
            (
                Symbol::new(&e, "ComplianceScoreUpdated"),
                commitment_id.clone(),
            ),
