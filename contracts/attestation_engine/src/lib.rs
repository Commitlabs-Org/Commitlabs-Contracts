...
/// Check if an address is a verifier (public version).
    ...
    ///
    /// # Parameters
    /// - `commitment_id`: Commitment identifier in `commitment_core`.
    ///
    /// # Returns
    /// - A `HealthMetrics` snapshot combining core commitment values with
    ///   attestation-derived fee totals, last drawdown, and volatility
    ///   exposure.
    ///
    /// # Security
    /// - Read-only function.
    /// - Treats `commitment_core` as the source of truth for commitment
    ///   existence and current value.
