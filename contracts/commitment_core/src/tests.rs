...
fn test_check_violations_no_violations() {
    ...
    let created_at = 1000u64;
    let commitment = build_test_commitment(
        &e,
        commitment_id,
        &owner,
        1000,
...
fn test_create_commitment_event() {
    ...
    // This test is left blank as asset transfer/mock-token dependencies are not handled here.
}
...
fn test_update_value_event() {
    ...
    let owner = Address::generate(&e);
    let updater = Address::generate(&e);
    create_test_commitment(&e, &contract_id, commitment_id.clone(), owner, 1000, 1000, 10);
...
fn test_update_value_rate_limit_enforced() {
    ...
    let owner = Address::generate(&e);
    let updater = Address::generate(&e);
    create_test_commitment(&e, &contract_id, commitment_id.clone(), owner, 100, 100, 10);
