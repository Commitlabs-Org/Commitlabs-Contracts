...
fn setup_contract(e: &Env) -> (Address, CommitmentNFTContractClient<'_>) {
    // initialize now calls admin.require_auth(); mock auth so every test that
    // calls this helper can proceed to initialize without explicit auth setup.
    e.mock_all_auths();
...
/// Returns the token_id. `caller` must be an authorized minter (e.g. admin).
fn mint_to_owner(
    e: &Env,
...
/// Verify that `initialize` calls `admin.require_auth()`.
///
/// Single deployer assumption: the transaction must be signed by the admin key.
/// This test uses `mock_all_auths()` and then inspects `e.auths()` to confirm
/// that the admin address was required to authorize the `initialize` invocation.
#[test]
fn test_initialize_requires_admin_auth() {
    ...
    let auths = e.auths();
    // At least one auth entry must be for admin.
    let found = auths.iter().any(|(addr, _)| *addr == admin);
    assert!(found, "admin must authorize initialize (single deployer assumption)");
}

/// `initialize` must store the admin that authorized the call.
///
/// Verifies that get_admin() returns the address that signed initialize,
/// not any other address.
#[test]
fn test_initialize_stores_authorizing_admin() {
    let e = Env::default();
    e.mock_all_auths();
    let (admin, client) = setup_contract(&e);
    client.initialize(&admin);
...
fn test_total_supply_unchanged_after_transfer_and_settle() {
    ...
    // Advance time past expiry (1 day = 86400 seconds)
    e.ledger().with_mut(|li| {
        li.timestamp = 86401;
    });

    client.settle(&token_id);
    assert_eq!(client.total_supply(), 1); // Total supply unchanged after settle

    // NFT is now inactive; transfer is allowed
    client.transfer(&owner1, &owner2, &token_id);
    assert_eq!(client.total_supply(), 1); // Total supply unchanged after transfer
...
// Active (locked) NFTs cannot be transferred — issue #145.
#[test]
...
fn test_transfer_locked_nft() {
    ...
    // Transferring an active (locked) NFT must fail with NFTLocked.
...
fn test_invariant_balance_sum_equals_supply_after_mints() {
    ...
    // Mint 4 to owner_a
    for i in 0..4 {
        mint_to_owner(&e, &client, &admin, &owner_a, &asset, &std::format!("a_{i}"));
        assert_balance_supply_invariant(&client, &owners);
    }
    ...
    // Mint 3 to owner_c
    for i in 0..3 {
        mint_to_owner(&e, &client, &admin, &owner_c, &asset, &std::format!("c_{i}"));
        assert_balance_supply_invariant(&client, &owners);
    }

    // Mint 2 to owner_d
    for i in 0..2 {
        mint_to_owner(&e, &client, &admin, &owner_d, &asset, &std::format!("d_{i}"));
        assert_balance_supply_invariant(&client, &owners);
    }
